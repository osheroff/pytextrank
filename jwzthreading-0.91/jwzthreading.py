...
def make_message_from_email (msg):
    """make_message(msg:rfc822.Message) : Message
    Create a Message object for threading purposes from an email.message 
    object
    """
    new = Message()

    m = msgid_pat.search(msg["Message-ID"] or "")
    if m is None:
        new.message_id = uuid.uuid4()
    else:
        new.message_id = m.group(1)
    ...
    new.references = msgid_pat.findall(refs)
    new.references = uniq(new.references)
    new.subject = msg["Subject"] or "No Subject"
